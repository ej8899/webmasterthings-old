...
<p>{$lang['search']} <input type="text" name="q"> {$lang['str_ing']} <select name="field" size="1">
...
?>
<br /><h1><?php echo str_replace("%num%",count($data), $lang['admin_posts_results']); ?></h1><br /><br />
<?php
...
EOF;
	...
	$date=date("d.m.Y",$results['date']);

echo <<<EOF
		<tr>
			<td headers="th0388AFB80000" valign="top" align="left">
			<a href="?action=edit_user&amp;id={$results['id']}" title="{$results['admin_user_edit']}">{$results['user_login']}</a>
			</td>
