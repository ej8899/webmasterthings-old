...
*/
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=="delete_comment" && isset($_REQUEST['id'])) {
		...
		if(is_numeric($_REQUEST['id'])) {
			$db->query("DELETE FROM ".MY_PRF."comments WHERE id='{$_REQUEST['id']}'");
		} else {
			// Delete all comments
		...
		if(defined('IN_ADMIN')) {
			...
			bmc_Go("Location: {$bmc_vars['c_urls']}/user.php?action=edit_comments&blog=".BLOG."&id=".$_REQUEST['post']);
		}
	}
	...
	if(isset($_POST['action']) && $_POST['action']=="mod_comment" && isset($_POST['id']) && is_numeric($_POST['id']) && defined('BLOG')) {
		...
		if(defined('IN_ADMIN')) {
			...
			bmc_Go("Location: {$bmc_vars['c_urls']}/user.php");
		}
	...
	if(defined('IN_ADMIN')) {
		$result=$db->query("SELECT m_cmt FROM ".MY_PRF."posts WHERE id='{$_REQUEST['id']}'", false);
	} else {
		$result=$db->query("SELECT m_cmt FROM ".MY_PRF."posts WHERE id='{$_REQUEST['id']}' AND author='{$logged_user_id}'", false);
		// An extra check to verify whether the current user is the actual poster
	}


	if(!$bmc_vars['m_cmt'] || !$result['m_cmt']) {
...
EOF;
	...
	<?php
		...
		if($comment['author']) {
			...
			echo " {$lang['str_on']} ".bmcDate($comment['date']);;
		}
	else {
