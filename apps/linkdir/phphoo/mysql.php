...
//	*****************************************************************
	...
	GLOBAL $wt_owner;
		...
		$sql = "SELECT CatID,CatName FROM $this->CAT_TBL WHERE CatParent $CatParent AND wt_owner $wt_owner ORDER BY CatName";
...
//	The power of recursive queries
	...
	GLOBAL $wt_owner;
		...
		$sql = "SELECT CatID,CatParent,CatName from $this->CAT_TBL where CatID = $CatID AND wt_owner =$wt_owner";
	...
	GLOBAL $wt_owner;
		...
		$sql = "SELECT CatID from $this->CAT_TBL where CatName = '$CatName' AND wt_owner '$wt_owner'";
		$results = $this->select($sql);
		if(!empty($results))
		{
	...
	GLOBAL $wt_owner;
        $cat = $this->CAT_TBL;
		$sql = "SELECT CatID,CatName FROM $cat WHERE wt_owner=$wt_owner";
		$results = $this->select($sql);
		return $results;
	}
	...
	GLOBAL $wt_owner;
		...
		$sql = "SELECT * FROM $this->LNK_TBL WHERE  (Approved != 0) AND CatID $CatID AND wt_owner=$wt_owner ORDER BY LinkName";
		$results = $this->select($sql);
		return $results;
	}
	...
	GLOBAL $wt_owner;
		...
		$sql = "SELECT DISTINCT LinkID,CatID,Url,LinkName,Description FROM $this->LNK_TBL WHERE (Approved != 0) AND wt_owner=$wt_owner AND ( $DEBUG ";
	...
	GLOBAL $wt_owner;
		...
		$sql .= "(CatID,Url,LinkName,Description,SubmitName,SubmitEmail,SubmitDate,Approved,wt_owner) ";
		$sql .= "values ";
		$sql .= "($CatID,'$Url','$LinkName','$Description','$SubmitName','$SubmitEmail',$SubmitDate,$Approved,$wt_owner) ";
