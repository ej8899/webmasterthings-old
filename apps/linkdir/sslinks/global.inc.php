...
/***********************************************************
*
* wt_wt_sslinks v1.22 - a PHP / mySQL links management system
* (c) Simon Willison 2001
* For more information, visit www.wt_sslinks.co.uk
*
***********************************************************/

// mySQL database Host / Name / Username / Password

$db_host = "$wtserver"; // Your mySQL server host address
$db_name = "$wtdatabase"; // The name of the database to use
$db_user = "$wtdb_user"; // Your mySQL username
$db_pass = "$wtdb_pass"; // Your mySQL password

// Admin username / password for the script

$admin_user = "Admin"; // This is the username used to log in as an admin
$admin_pass = "scubag"; // This is the password used to log in as an admin

// Files containing header and footer HTML, also name of the site

$sitename = "wt_sslinks"; 		// Set this to the name of your site
$sitemail = "wtlinks@webmasterthings.com"; 	// Set to your e-mail address.
...
$template_dir = "templates/"; // Set this to the directory that templates are stored in.
		       // Alternatively leave it blank if templates in same dir as wt_sslinks scripts
...
{ 
	...
	else
	{
		...
		if ($debug) echo "<!--Loading Template: $template_fullpath (wtownerinfo|$wtownerinfo|)-->";
	...
	if (is_array($replacements))
	{
		...
		$contents = eregi_replace('%wt_owner%', $wtownerinfo, $contents);
	return $contents;
}
...
function numlinks_array()
{
	...
	$sql = "SELECT wt_sslinkcats.lcat_id, COUNT(*) AS NumLinks, lcat_numlinks
		FROM wt_sslinks, wt_sslinkcats 
		WHERE link_cat = wt_sslinkcats.lcat_id 
...
function cats_table($id)
{
	...
	$result = mysql_query("SELECT lcat_id, lcat_name, lcat_ranking, lcat_header FROM wt_sslinkcats WHERE lcat_cat = '$id' AND wt_owner=$wtownerinfo ORDER BY lcat_ranking DESC, lcat_name");
	if (!$result)
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$html .= ss_template('categories_item.tmpl',array('%catid%' => $catid, '%catname%' => $catname, '%catdesc%' => $thecat_header,  '%cat_numlinks%' =>  $lcat_numlinks, '%cat_numlinkstree%' => $lcat_numlinkstree, '%admin_options%' => $admin_html));
		if (is_int($i / 2))
...
function cats_sql($sql)
{
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$html .= ss_template('categories_item.tmpl',array('%catid%' => $catid, '%catdesc%'=>$catdesc, '%catname%' => $catname, '%cat_numlinks%' => $lcat_numlinks, '%cat_numlinkstree%' => $lcat_numlinkstree, '%admin_options%' => $admin_html));
		if (is_int($i / 2))
			$html .= ss_template('categories_list_nextrow.tmpl');
		$i++;
	}
...
function get_catheader($id)
{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("SELECT lcat_header FROM wt_sslinkcats WHERE lcat_id = '$id'");
	if (!$result)
...
function get_catname($id)
{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("SELECT lcat_name FROM wt_sslinkcats WHERE lcat_id = '$id'");
	if (!$result)
...
function show_links($id)
{
	...
	$result1 = mysql_query("SELECT * FROM wt_sslinks WHERE link_cat = '$id' AND link_validated = 'yes' AND link_recommended = 'yes' AND wt_owner=$wtownerinfo ORDER BY link_name");
	if (!$result1)
		custom_die("SQL result failed to get links: $result1");
	$num1 = mysql_num_rows($result1);
	$result2 = mysql_query("SELECT * FROM wt_sslinks WHERE link_cat = '$id' AND link_validated = 'yes' AND link_recommended = 'no' AND wt_owner=$wtownerinfo ORDER BY link_name");
	if (!$result2)
		custom_die("SQL result failed");
...
function jump_to($id)
{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("SELECT link_url, link_hits FROM wt_sslinks WHERE link_id = '$id'");
	if (!$result)
		custom_die("SQL result failed");
	$num = mysql_num_rows($result);
	if ($num == 0)
	{
		header("Location: links.php?wt_owner=$wtownerinfo");
		exit;
	}
	while ($row = mysql_fetch_array($result))
	{
	...
	$hits++;
	$result2 = @mysql_query("UPDATE wt_sslinks SET link_hits = '$hits' WHERE link_id = '$id'");
	header("Location: $url");
	exit;
}
...
function admin_options($cat)
{
	...
	{
		$delcat = " - <a href=\"links.php?action=delcat&wt_owner=$wt_owner&id=$cat\">Delete This Category</a>";
	}
	else
	{
		$delcat = "";
...
function do_rating($id, $score)
{
	...
	global $$cookie;	global $wt_owner,$wtownerinfo;
	$cnx = mysql_connect($db_host, $db_user, $db_pass)
		or custom_die("Unable to connect to database server.");
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("SELECT link_totalrate, link_numvotes, link_cat FROM wt_sslinks WHERE link_id = '$id'");
	if (!$result)
		custom_die("SQL result failed");
	$num = mysql_num_rows($result);
	if ($num == 0)
	...
	if ($$cookie == "yes")
	{
		header("Location: links.php?cat=$cat&wt_owner=$wtownerinfo");
		exit;
	}
	$numvotes++;
	$totalrate = $totalrate + $score;
	$result2 = @mysql_query("UPDATE wt_sslinks SET link_numvotes = '$numvotes', link_totalrate = '$totalrate' WHERE link_id = '$id'");
	setcookie($cookie, "yes", time() + 360000);
	header("Location: links.php?cat=$cat&wt_owner=$wt_owner");
	exit;	
}

function add_link($link_name, $link_url, $link_desc, $link_cat, $link_recommended,$wt_owner,$password)
{
	global $wt_owner,$wtownerinfo;
	...
	if (!$admin)
	{
		header("Location: links.php?action=logout&wt_owner=$wtownerinfo");
		exit;
	}
	if (!$link_recommended)
		$link_recommended = "no";
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$time = time();
	$sql = "INSERT INTO wt_sslinks SET
		link_name = '$link_name',
		link_cat = '$link_cat',
		link_url = '$link_url',
		link_desc = '$link_desc',
	...
	else
	{
		...
		build_numlinks();
		header("Location: links.php?cat=$link_cat&wt_owner=$wtownerinfo");
		exit;
	}
}


function add_cat($lcat_name, $lcat_header, $lcat_cat, $lcat_ranking,$wt_owner)
{
	global $wtownerinfo;
	// adds a link to the database
	global $db_host, $db_name, $db_user, $db_pass, $admin, $sitename;
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$sql = "INSERT INTO wt_sslinkcats SET
		lcat_name = '$lcat_name',
		lcat_cat = '$lcat_cat',
	...
	else
	{
		// Rebuild number of links for categories
		build_numlinks();
		header("Location: links.php?cat=$lcat_cat&wt_owner=$wtownerinfo");
		exit;
	}	
}
...
function do_breadcrumbs($cat)
{
	...
	*/
	...
	while ($lcat != 0)
	{
		$sql = "SELECT lcat_cat, lcat_name, lcat_id FROM wt_sslinkcats WHERE lcat_id = '$lcat'";
		$result = mysql_query($sql);
		if (!$result)
		{
...
function del_cat($id)
{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$sql = "DELETE FROM wt_sslinkcats WHERE lcat_id = '$id'";
	$result = mysql_query($sql);
	if (!$result)
	{
...
function delete_link($id)
{
	...
	if (!$admin)
	{
		header("Location: admin.php?action=logout&wt_owner=$wtownerinfo");
		exit;
	}
	...
	if (!$submit)
	{
		...
		mysql_select_db($db_name, $cnx)
			or custom_die("Unable to select database.");
		$result = mysql_query("SELECT * FROM wt_sslinks WHERE link_id = '$id'");
		if (!$result)
			custom_die("SQL result failed");
		while ($row = mysql_fetch_array($result))
		{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("DELETE FROM wt_sslinks WHERE link_id = '$id'");
	if (!$result)
		custom_die("SQL result failed");
	// Rebuild number of links for categories
	build_numlinks();
...
function reject_link($id)
{
	...
	$result2 = mysql_query("DELETE FROM wt_sslinks WHERE link_id = '$id'");
	if (!$result2)
		custom_die("SQL result failed");
	// Rebuild number of links for categories
	build_numlinks();
...
function validate_list()
{
	...
	mysql_select_db($db_name, $cnx)
		or custom_die("Unable to select database.");
	$result = mysql_query("SELECT * FROM wt_sslinks WHERE link_validated = 'no' AND wt_owner=$wtownerinfo ORDER BY link_name");
	if (!$result)
		custom_die("SQL result failed");
	$html = "";
	$num = mysql_num_rows($result);
...
function val_link($id)
{
	...
	$time = time();
	$result2 = mysql_query("UPDATE wt_sslinks SET link_validated = 'yes', link_dateadd = '$time' WHERE link_id = '$id'");
	if (!$result2)
		custom_die("SQL result failed");
	if ($link_addemail != "")
	{
	...
	header("Location: links.php?action=validated&wt_owner=$wtownerinfo");
	exit;
}
...
function search_results($search)
{
	...
	$links_sql = "SELECT * FROM wt_sslinks WHERE link_validated = 'yes' AND wt_owner=$wtownerinfo AND
		(link_name LIKE '%$search%' OR link_desc LIKE '%$search%')
		ORDER BY link_name";
	$cats_sql = "SELECT lcat_id, lcat_name, lcat_ranking,lcat_header FROM wt_sslinkcats WHERE wt_owner=$wtownerinfo AND 
	...
	echo ss_template('search_results.tmpl', array('%cats%' => $cats, '%catdesc%' => $catdesc, '%links%' => $links, '%searchtext%' => $searchtext));
...
function new_links($numdays)
{
	...
	$links_sql = "SELECT * FROM wt_sslinks WHERE link_dateadd > $mintime AND link_validated = 'yes' AND wt_owner=$wtownerinfo ORDER BY link_dateadd DESC LIMIT 0,$nol";
	$links = links_sql($links_sql);
...
function popular_links()
{
	...
	$links_sql = "SELECT * FROM wt_sslinks WHERE link_validated = 'yes' AND wt_owner=$wtownerinfo AND link_hits > 0 ORDER BY link_hits DESC LIMIT 0,$nol";
	$links = links_sql($links_sql);
...
function top_rated()
{
	...
	global $minvotes, $nol;
	$links_sql = "SELECT *, (link_totalrate/link_numvotes) AS link_rating FROM wt_sslinks WHERE link_validated = 'yes' AND wt_owner=$wtownerinfo AND link_numvotes > $minvotes ORDER BY link_rating DESC LIMIT 0,$nol";
	$links = links_sql($links_sql);
...
function edit_cat($id)
{
	...
	if (!$submit)
	{
		...
		mysql_select_db($db_name, $cnx)
			or custom_die("Unable to select database.");
		$result = mysql_query("SELECT * FROM wt_sslinkcats WHERE lcat_id = '$id'");
		if (!$result)
			custom_die("SQL result failed");
		while ($row = mysql_fetch_array($result))
		{
...
function category_numlinks_cache ($cat)
{
	...
	*/
	...
	$sql = "SELECT lcat_cat, lcat_id FROM wt_sslinkcats WHERE lcat_cat = '$cat'";
	$result = mysql_query($sql);
