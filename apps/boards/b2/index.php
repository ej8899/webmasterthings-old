...
$wt_owner			=GetVars("wt_owner");	// tcmMod

if(!$wt_owner)		// set a default to a general chat board
	{ $wt_owner='0'; }
...
// insert the wt header here
// echo "$wt_header";
include ($wtdb[headerfile]);		// changed 26 sept 03 - should solve not loading it all problem?


/* Display the page header, including CSS stuff */
echo  "    <TITLE>$wtdb[sitename] Message Board [ powered by WebmasterThings.com - board " . VERSION . " ]</TITLE>\n"
   . "    <STYLE>\n"
   . "      A\n"
   . "        {\n"
   . "          color: " . LINK_COLOR . ";\n"
...
echo "    <table border=0 bgcolor=\"#585858\" cellpadding=2 cellspacing=0 width=600><tr><td><table border=0 width=100% bgcolor=white cellpadding=0 cellspacing=0><tr><td><TABLE align=\"center\" valign=\"top\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" bgcolor=". HEADER_BACKGROUND . ">\n";

/* Check if the user is logged in */
if ($logged_in == 0)
  {
    /* If not, then display the 'Log In' option */
    $login_status = "Not logged in (<A href=\"?wt_owner=$wt_owner&pid=login\">Log In</A>)";
  }
else
  {
    ...
    $login_status = "Logged in as <B>$username</B> (<A href=\"?wt_owner=$wt_owner&pid=login&logout=now\">Log Out</A>)";
  }
...
echo "                            \n"
   ...
   . "                  <TR>\n"
   . "                    <TD valign=\"middle\"><FONT class=\"small_text\"><a href=\"$wtdb[siteurl]\"><strong>$wtdb[sitename]</strong></a>"; ?> <?php include("http://www.webmasterthings.com/apps/counter/counter.php?name=cbbs"); ?> <?php echo "<BR><BR></font>\n";

/* If not logged in, give the register link */
if ($logged_in == 0)
  {
    $show_profile  = "";
    $show_register = "<A href=\"?wt_owner=$wt_owner&pid=register\">Register</A> | ";
  }
/* If logged in, then give a link to their profile */
else
  {
    $show_profile  = "<A href=\"?wt_owner=$wt_owner&pid=edit_profile\">My Profile</A> | ";
    $show_register = "";
  }
...
echo "                      <FONT class=\"small_text\">&nbsp;$show_profile$show_register$show_admin <A href=\"?wt_owner=$wt_owner&pid=faq\">FAQ</A> | <a href=\"?wt_owner=$wt_owner\">Home</a><!-- | Search --></FONT><BR>\n"
   . "                    </TD><td valign=bottom align=right><FONT class=\"small_text\"><BR>$login_status</FONT></td>\n"
   . "                  </TR>\n"
   . "                </TABLE>\n"
   . "              </TD\n"
   . "            </TR>\n"
...
// echo "$wt_footer";
include ($wtdb[footerfile]);	// added 26 sept 03
