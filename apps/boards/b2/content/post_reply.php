...
if (!(in_array($thread_id, $thread_list)))
  {
    ...
    echo "            <CENTER class=\"error_message\">Malformed request detected!<BR>Invalid Thread!</CENTER><BR>\n";
...
if ($correct_forum != $forum_id)
  {
    ...
    echo "            <CENTER class=\"error_message\">Malformed request detected!<BR>Forum identification error!</CENTER><BR>\n";
    require ("./content/view_forums.php");
    return;
  }
...
switch ($step)
  {
    ...
    default:
    case 1:
      ShowPostReplyForm( $username, $password, $email, $message, $include_sig, $user_id, $thread_id, $forum_id,$wt_owner,$picture1url );
      break;
    ...
    case 2:
      ...
      echo "                    $display_message\n"
         ...
         . "              <INPUT type=\"hidden\" name=\"user_id\" value=\"$user_id\"><input type=hidden name=wt_owner value=$wt_owner>\n"
         . "              <CENTER>\n"
         . "                <BR>\n"
...
// returns with $newfilename set
          ...
          $SQL     = "INSERT INTO " . TABLE_PREFIX . "replies (reply_body, user_id, user_ip, thread_id, forum_id,attachment) VALUES ('$message', '$user_id', '$user_ip', '$thread_id', '$forum_id','$newfilename');";
          $results = ExeSQL($SQL);
      ...
      else
        {
          ...
          ShowPostReplyForm( $username, $password, $email, $message, $include_sig, $user_id, $thread_id, $forum_id, $db_name, $connection,$wt_owner,$picture1url );
        }
      break;
  }
...
ShowPostReplyForm( $username, $password, $email, $message, $include_sig, $user_id, $thread_id, $forum_id,$wt_owner,$picture1url )
{
  ...
  echo "            <SCRIPT language=\"JavaScript\">\n"
     ...
     . "              }\n"
     . "            </SCRIPT>\n"
     . "            <FORM action=\"?pid=post_reply&step=3\" enctype=\"multipart/form-data\" method=\"POST\" name=\"post_reply\">\n"
     . "              <TABLE cellpadding=\"5\" cellspacing=\"0\" width=\"100%\" border class=\"table_border\">\n"
     . "                <TR class=\"table_header\">\n"
     ...
     . "                  <TD width=\"50%\" nowrap><input type=\"file\" name=\"picture1url\" size=\"25\" class=\"input\"></TD>\n"
     . "                </TR>\n"		 
     . "                <TR bgcolor=\"" . TABLE_COLOR_1 . "\" class=\"regular_text\">\n"
  ...
  echo "                    </TABLE>\n"
     ...
     . "              <CENTER><BR><INPUT type=\"Submit\" value=\"Post Reply\" onClick=\"return CheckForm();\"></CENTER>\n"
     . "            </FORM>\n"; // change form url to step=2 for preview
}
