...
while ($row = mysql_fetch_array($results))
  echo "                <TD class=\"regular_text\"><BR> <A href=\"?wt_owner=$wt_owner&pid=view_forums\">All Forums</A> > " . $row["forum_name"]."</TD>\n";
...
if ($any_threads != 0)
  {
    ...
    echo "                <TD align=\"right\" class=\"regular_text\"><BR><A href=\"?wt_owner=$wt_owner&pid=post_thread&forum_id=$forum_id\">Post New Thread</A></TD>\n"
       . "              </TR>\n"
       . "            </TABLE>\n"
       . "            <BR>\n";
	...
	/* Pull each user name from the database */
	  ...
	  $the_user = $row2["user_name"];
        ...
        echo "              <TR>\n"
           ...
           . "                  <IMG src=\"./images/$which_image.png\"> <A href=\"?wt_owner=$wt_owner&pid=view_replies&thread_id=" . $row["thread_id"] . "&forum_id=$forum_id\">" . $row["thread_title"] . "</A>\n"
           . "                </TD>\n"
           . "                <TD bgcolor=\"" . TABLE_COLOR_2 . "\" align=\"center\" nowrap class=\"small_text\">\n"
           . "                  <A href=\"?wt_owner=$wt_owner&pid=view_profile&user=" . $the_user . "\">" . $the_user . "</A>\n"
           . "                </TD>\n"
...
else
  {
    ...
    echo "              </TR>\n"
       ...
       . "              <A href=\"?wt_owner=$wt_owner&pid=post_thread&forum_id=$forum_id\">Click here if you'd like to post a new thread.</A><BR><BR>\n"
       . "            </CENTER>\n";
  }
