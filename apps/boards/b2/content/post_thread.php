...
switch ($step)
  {
    ...
    default:
    case 1:
      ShowPostThreadForm( $username, $password, $email, $title, $message, $include_sig, $user_id, $forum_id,$wt_owner,$picture1url );
      break;

    /* Display the thread for the user to preview */
    case 2:
      /* Display the top part */
      echo "            <FORM action=\"?pid=post_thread\" enctype=\"multipart/form-data\" method=\"POST\" name=\"post_thread\">\n"
      ...
      echo "                    $display_message\n"
         ...
         . "                </TR>\n"
         . "              </TABLE>\n"
		 . "				<input type=hidden name=picture1url value=\"$picture1url\">\n"
...
// returns with $newfilename set
          ...
          $SQL     = "INSERT INTO " . TABLE_PREFIX . "threads (thread_title, thread_body, user_id, user_ip, forum_id,attachment) VALUES ('$title', '$message', '$user_id', '$user_ip', '$forum_id','$newfilename');";
          $results = ExeSQL($SQL);
...
ShowPostThreadForm( $username, $password, $email, $title, $message, $include_sig, $user_id, $forum_id,$wt_owner,$picture1url )
{
  ...
  echo "            <SCRIPT language=\"JavaScript\">\n"
     ...
     . "              }\n"
     . "            </SCRIPT>\n"
     . "            <FORM action=\"?pid=post_thread&step=3\" enctype=\"multipart/form-data\" method=\"POST\" name=\"post_thread\">\n"
     . "              <TABLE cellpadding=\"5\" cellspacing=\"0\" width=\"100%\" border class=\"table_border\">\n"
     . "                <TR class=\"table_header\">\n"
     ...
     . "                  <TD width=\"50%\" nowrap><input type=\"file\" name=\"picture1url\" size=\"25\" class=\"input\" value=\"$picture1url\"></TD>\n"
     . "                </TR>\n"	 
     . "                <TR bgcolor=\"" . TABLE_COLOR_2 . "\" class=\"regular_text\">\n"
  ...
  echo "                    </TABLE>\n"
     ...
     . "              <CENTER><BR><INPUT type=\"Submit\" value=\"Post Thread\" onClick=\"return CheckForm();\"></CENTER>\n"
     . "            </FORM>\n"; // change Post Thread to Preview Thread to engage preview mode
}
