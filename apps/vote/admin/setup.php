...
function setup_check_dbtables() {
   ...
   $q2 = "SELECT uid FROM wt_sympoll_auth WHERE(access='0')";
...
function setup_process_dbtables($tbc) { 
   ...
   <?php 
   $q['wt_sympoll_data']  = "CREATE TABLE wt_sympoll_data (pid INT UNSIGNED NOT NULL, ";
   $q['wt_sympoll_data'] .= "cid INT UNSIGNED NOT NULL, choice VARCHAR(250) NOT NULL, ";
   $q['wt_sympoll_data'] .= "votes INT UNSIGNED DEFAULT '0' NOT NULL, INDEX(pid))";

   $q['wt_sympoll_list']  = "CREATE TABLE wt_sympoll_list (pid INT UNSIGNED NOT NULL AUTO_INCREMENT, ";
   $q['wt_sympoll_list'] .= "nextcid INT UNSIGNED DEFAULT '0' NOT NULL, question ";
   $q['wt_sympoll_list'] .= "VARCHAR(250) NOT NULL, timeStamp INT UNSIGNED NOT NULL, cookieStamp ";
   $q['wt_sympoll_list'] .= "INT UNSIGNED NOT NULL, status SMALLINT UNSIGNED NOT NULL, ";
   $q['wt_sympoll_list'] .= "PRIMARY KEY (pid))";

   $q['wt_sympoll_auth']  = "CREATE TABLE wt_sympoll_auth (uid INT NOT NULL AUTO_INCREMENT, ";
   $q['wt_sympoll_auth'] .= "user VARCHAR(32) NOT NULL, pass VARCHAR(32) NOT NULL, access ";
   $q['wt_sympoll_auth'] .= "SMALLINT UNSIGNED NOT NULL, secret VARCHAR(32), PRIMARY KEY (uid))";

   $q['wt_sympoll_iplog']  = "CREATE TABLE wt_sympoll_iplog (vid VARCHAR(32) NOT NULL, ";
   $q['wt_sympoll_iplog'] .= "pid INT UNSIGNED NOT NULL, voted INT UNSIGNED NOT NULL, ";
   $q['wt_sympoll_iplog'] .= "INDEX (vid), INDEX (pid))";
...
function upgrade_process_dbtables($gen) {
   ...
   <?php switch($gen) {
      case 1:
         ...
         upgrade_rename_col("wt_sympoll_list", "pollID", "pid", "INT NOT NULL AUTO_INCREMENT");
         upgrade_rename_col("wt_sympoll_data", "pollID", "pid", "INT NOT NULL");
         upgrade_rename_col("wt_sympoll_data", "choiceID", "cid", "INT NOT NULL");
         upgrade_rename_col("wt_sympoll_auth", "id", "uid", "INT NOT NULL AUTO_INCREMENT");
         addc_nextcid_symplist();
      case 2:
         echo "<br /><b>(generation 2)</b><br />";
         upgrade_rename_col("wt_sympoll_auth", "user", "user", "VARCHAR(32) NOT NULL");
         upgrade_rename_col("wt_sympoll_auth", "pass", "pass", "VARCHAR(32) NOT NULL");
         upgrade_purge_table("wt_sympoll_auth"); 
      case 3:
         echo "<br /><b>(generation 3)</b><br />";
         upgrade_add_index("wt_sympoll_data", "pid");
      case 4:
         echo "<br /><b>(generation 4)</b><br />";
         upgrade_delete_col("wt_sympoll_list", "identifier");
         upgrade_rename_col("wt_sympoll_data", "pid", "pid", "INT UNSIGNED NOT NULL");
         upgrade_rename_col("wt_sympoll_data", "cid", "cid", "INT UNSIGNED NOT NULL");
         upgrade_rename_col("wt_sympoll_data", "votes","votes", "INT UNSIGNED DEFAULT '0' NOT NULL");
         upgrade_rename_col("wt_sympoll_list", "pid", "pid", "INT UNSIGNED NOT NULL AUTO_INCREMENT");
         upgrade_rename_col("wt_sympoll_list", "timeStamp", "timeStamp", "INT UNSIGNED NOT NULL");
         upgrade_rename_col("wt_sympoll_auth", "access", "access", "SMALLINT UNSIGNED NOT NULL");
         upgrade_rename_col("wt_sympoll_iplog", "pid", "pid", "INT UNSIGNED NOT NULL");
         upgrade_rename_col("wt_sympoll_iplog", "voted", "voted", "INT UNSIGNED NOT NULL");
...
function addc_nextcid_symplist() {
   ...
   $fields = mysql_list_fields($s_dbname, "wt_sympoll_list", $s_dbid);
   ...
   Altering wt_sympoll_list in <?php echo $s_dbname; ?>: adding 'nextcid'...
   <?php $q1 = "ALTER TABLE wt_sympoll_list ADD nextcid INT UNSIGNED DEFAULT '0' NOT NULL AFTER identifier";
   $q2 = "SELECT pid FROM wt_sympoll_list";
   $r1 = mysql_query($q1, $s_dbid) or die("<b>FAILED!</b> (lacking alter permissions?)<br />");
   $r2 = mysql_query($q2, $s_dbid);
   for($dsr = 0; $dsr < mysql_num_rows($r2); $dsr++) {
      $a2 = mysql_fetch_array($r2);
      $q3 = "SELECT MAX(cid)+1 AS nextc FROM wt_sympoll_data WHERE pid='$a2[pid]'";
      $r3 = mysql_query($q3, $s_dbid);
      /* if update fails revert back */
      if(!$r3) {
         $q4 = "ALTER TABLE wt_sympoll_list DROP nextcid";
         $r4 = mysql_query($q4, $s_dbid);
         die("<b>FAILED!</b> (unable to calculate nextcid?)<br />");
      }
      $a3 = mysql_fetch_array($r3);
      $q5 = "UPDATE wt_sympoll_list SET nextcid='$a3[nextc]' WHERE pid='$a2[pid]'";
      $r5 = mysql_query($q5, $s_dbid);
      /* if update fails revert back */
      if(!$r5) {
         $q6 = "ALTER TABLE wt_sympoll_list DROP nextcid";
...
function addc_cookiestamp_symplist() {
   ...
   Altering wt_sympoll_list in <?php echo $s_dbname; ?>: adding 'cookieStamp'...
   <?php $q1 = "ALTER TABLE wt_sympoll_list ADD cookieStamp INT UNSIGNED NOT NULL AFTER timeStamp";
   $r1 = mysql_query($q1, $s_dbid) or die("<b>FAILED!</b> (lacking alter permissions?)<br />");
   $q2 = "UPDATE wt_sympoll_list SET cookieStamp=timeStamp";
   $r2 = mysql_query($q2, $s_dbid);

   /* if update fails revert back */
   if(!$r2) {
      $q3 = "ALTER TABLE wt_sympoll_list DROP cookieStamp";
      $r3 = mysql_query($q3, $s_dbid);
      die("<b>FAILED!</b> (lacking update permissions?)<br />");
   } ?>
   <b>done!</b><br />
...
function fixc_ip_sympiplog() {
   ...
   $fields = mysql_list_fields($s_dbname, "wt_sympoll_iplog", $s_dbid);
   ...
   Altering wt_sympoll_iplog in <?php echo $s_dbname; ?>: fixing 'ip'...
   <?php /* check for problems */
   if(!$has_old) {
      die("<b>FAILED!</b> (ip does not exist?)<br />");
   }
   /* do rename */
   $q1 = "ALTER TABLE wt_sympoll_iplog CHANGE ip vid VARCHAR(32) NOT NULL";
   $r1 = mysql_query($q1, $s_dbid) or die("<b>FAILED!</b> (lacking alter permissions?)<br />");

   /* do fix */
   $q2 = "UPDATE wt_sympoll_iplog SET vid=MD5(vid)";
   $r2 = mysql_query($q2, $s_dbid);

   /* if update fails revert back */
   if(!$r2) {
      $q3 = "ALTER TABLE wt_sympoll_iplog CHANGE vid ip VARCHAR(32) NOT NULL";
      $r3 = mysql_query($q3, $s_dbid);
      die("<b>FAILED!</b> (lacking update permissions?)<br />");
   } ?>
...
function fixc_status_symplist() {
   ...
   $has_old = FALSE; 
   $q1 = "SHOW COLUMNS FROM wt_sympoll_list";
   ...
   Altering wt_sympoll_list in <?php echo $s_dbname; ?>: fixing 'status'...
   <?php /* check for problems */
   if(!$has_old) {
      die("<b>FAILED!</b> (status does not exist?)<br />");
   }

   $q2 = "ALTER TABLE wt_sympoll_list CHANGE status status SMALLINT UNSIGNED NOT NULL";
   $r2 = mysql_query($q2, $s_dbid) or die("<b>FAILED!</b> (lacking alter permissions?)<br />");
   $new = $S_VIEW_OPEN + $S_TYPE_RADIO;
   $q3 = "UPDATE wt_sympoll_list SET status='$new' WHERE status='1'";
   $r3 = mysql_query($q3, $s_dbid);
   if(!$r3) {
      $q5 = "ALTER TABLE wt_sympoll_list CHANGE status status INT NOT NULL";
      $r5 = mysql_query($q5, $s_dbid);
      die("<b>FAILED!</b> (lacking update permissions?)<br />");
   }
   $new = $S_VIEW_HIDDEN + $S_TYPE_RADIO;
   $q4 = "UPDATE wt_sympoll_list SET status='$new' WHERE status='0'";
   $r4 = mysql_query($q4, $s_dbid);
   ?> <b>done!</b><br />
<?php }
