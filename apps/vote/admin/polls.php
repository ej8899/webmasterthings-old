...
function display_log($pid) {
   ...
   if($s_iplog != "0") {
      $q1 = "SELECT vid,voted FROM wt_sympoll_iplog WHERE(pid='$pid') ORDER BY voted DESC";
      $r1 = mysql_query($q1, $s_dbid);
      $q2 = "SELECT vid,COUNT(pid) AS count FROM wt_sympoll_iplog GROUP BY vid";
...
function process_create($question, $view, $type, $newo) {
   ...
   /* insert poll info */
   $time = time();
   $q1 = "INSERT INTO wt_sympoll_list VALUES(NULL, 0, '$question', '$time', '$time', '$status')";
   $r1 = mysql_query($q1, $s_dbid);
   $q2 = "SELECT pid FROM wt_sympoll_list WHERE(timeStamp='$time')";
   $r2 = mysql_query($q2, $s_dbid);
   ...
   while(isset($newo) && list($k,$v) = each($newo)) {
      if(trim($v) != "") {
         $v = addslashes($v);
         $q3 = "INSERT INTO wt_sympoll_data VALUES('$a2[pid]', '$k', '$v', 0)";
   ...
   $maxcid++;
   $q4 = "UPDATE wt_sympoll_list SET nextcid='$maxcid' WHERE pid='$a2[pid]'";
...
function process_edit($pid, $question, $view, $type, $newo, $updateo, $deleteo) {
   ...
   /* update the question */
   $q1 = "UPDATE wt_sympoll_list SET question='$question',status='$status' WHERE(pid='$pid')";
   $r1 = mysql_query($q1, $s_dbid);
   ...
   while(isset($newo) && list($k,$v) = each($newo)) {
      if(trim($v) != "") {
         $v = addslashes($v);
         $q2 = "INSERT INTO wt_sympoll_data VALUES('$pid', '$k', '$v', 0)";
         $r2 = mysql_query($q2, $s_dbid);
         if($k > $maxcid)  {
            $maxcid = $k; 
         }
   ...
   if($maxcid > -1) {
      $maxcid++;
      $q3 = "UPDATE wt_sympoll_list SET nextcid='$maxcid' WHERE(pid='$pid')";
      $r3 = mysql_query($q3, $s_dbid);
   }
   ...
   while(isset($updateo) && list($k,$v) = each($updateo)) {
      if(trim($v) != "") {
         $v = addslashes($v);
         $q4 = "UPDATE wt_sympoll_data SET choice='$v' WHERE(pid='$pid' AND cid='$k')";
         $r4 = mysql_query($q4, $s_dbid);
      }
   }
   ...
   while(isset($deleteo) && list($k,$v) = each($deleteo)) {
      if($v == 'poof') {
         $q5 = "DELETE FROM wt_sympoll_data WHERE(pid='$pid' AND cid='$k')";
...
function process_del_rs($pid, $confirmation) {
   ...
   if($confirmation == 'delete') {
      $q1 = "DELETE FROM wt_sympoll_list WHERE(pid='$pid')";
      $r1 = mysql_query($q1, $s_dbid);
      $q2 = "DELETE FROM wt_sympoll_data WHERE(pid='$pid')";
      $r2 = mysql_query($q2, $s_dbid);
      $q3 = "DELETE FROM wt_sympoll_iplog WHERE(pid='$pid')";
      $r3 = mysql_query($q3, $s_dbid);
      $g_message = "Poll #$pid deleted";
   } else if($confirmation == 'reset') {
      $time = time();
      $q4 = "UPDATE wt_sympoll_list SET cookieStamp='$time' WHERE(pid='$pid')";
      $r4 = mysql_query($q4, $s_dbid);
      $q5 = "UPDATE wt_sympoll_data SET votes='0' WHERE(pid='$pid')";
      $r5 = mysql_query($q5, $s_dbid);
      $q6 = "DELETE FROM wt_sympoll_iplog WHERE(pid='$pid')";
...
function process_log($pid, $purgecookie, $purgeip) {
   ...
   if($purgecookie == 'poof') {
      $time = time();
      $q1 = "UPDATE wt_sympoll_list SET cookieStamp='$time' WHERE(pid='$pid')";
      $r1 = mysql_query($q1, $s_dbid);
   ...
   if($purgeip == 'poof') {
      $q2 = "DELETE FROM wt_sympoll_iplog WHERE(pid='$pid')";
      $r2 = mysql_query($q2, $s_dbid);
      if(!isset($g_message) || $g_message == "") {
