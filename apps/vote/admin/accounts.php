...
function auth_cookie() {
   ...
   $q1  = "SELECT uid FROM wt_sympoll_auth ";
...
function auth_user($user, $pass) {
   ...
   $q1  = "UPDATE wt_sympoll_auth SET secret='$md5' ";
...
function verify_access($access) {
   ...
   $q1  = "SELECT uid FROM wt_sympoll_auth WHERE(user='$sympauth[1]' "; 
...
function display_rmuser() {
   ...
   $q1 = "SELECT uid,user FROM wt_sympoll_auth WHERE(access != 0) ORDER BY uid";
   $r1 = mysql_query($q1, $s_dbid);
...
function process_adduser($user, $pass1, $pass2, $first) {
   ...
   if($first) {
      $q3 = "SELECT uid FROM wt_sympoll_auth WHERE(access='0')";
   ...
   $q1 = "SELECT user FROM wt_sympoll_auth WHERE(user='$user')";
   $r1 = mysql_query($q1, $s_dbid);
   ...
   $q2  = "INSERT INTO wt_sympoll_auth (uid, user, pass, access) ";
...
function process_rmuser($uid) {
   ...
   $q1 = "SELECT user,access FROM wt_sympoll_auth WHERE(uid='$uid')";
   $r1 = mysql_query($q1, $s_dbid);
   ...
   $q2 = "DELETE FROM wt_sympoll_auth WHERE(uid='$uid')";
   $r2 = mysql_query($q2, $s_dbid);
   if(mysql_affected_rows($s_dbid) <= 0) {
...
function process_pass($oldpass, $newpass1, $newpass2) {
   ...
   $opass = md5($oldpass);
   $npass = md5($newpass1);
   $q1  = "UPDATE wt_sympoll_auth SET pass='$npass' WHERE";
