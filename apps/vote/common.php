...
if(!function_exists('symp_poll_fetch')) {
   ...
   function symp_poll_fetch($pid) {
      ...
      if($pid == -1) {
         $q1 = "SELECT pid FROM wt_sympoll_list WHERE (status & '$S_VIEW_OPEN')";
         ...
         $q2  = "SELECT pid FROM wt_sympoll_list WHERE (status & '$S_VIEW_OPEN') ORDER BY ";
      ...
      $q3  = "SELECT * FROM wt_sympoll_list,wt_sympoll_data WHERE (wt_sympoll_list.pid";
      $q3 .= "='$pid' AND wt_sympoll_data.pid='$pid') ORDER BY cid";
      ...
      if($s_cookielog != "0" && isset($sympvotes["symp$p[cstamp]"])) {
         ...
         $q4  = "SELECT voted FROM wt_sympoll_iplog WHERE (vid='$SYMP_REMOTE_ADDR' ";
         $q4 .= "AND pid='$pid')";
         ...
         if(mysql_num_rows($r4) > 0) {
            ...
            if($now > ($a4['voted'] + ($s_blength * 86400)) ) {
               $q5  = "DELETE FROM wt_sympoll_iplog WHERE (vid='$SYMP_REMOTE_ADDR' ";
               $q5 .= "AND pid='$pid')";
